...
class ChatBox:
        ...
        with ui.card().classes('w-full shadow-none rounded-none'):
            with ui.column().classes('w-full gap-0'):
                ...
                self.text_area = ui.textarea(placeholder='Type your message...') \
                    ...
                    .props('filled autogrow input-class="h-auto"')
                ...
                with ui.row().classes('w-full items-center justify-between p-2'):
                    ...
                    ui.button(icon='send', on_click=self._handle_main_submit).props('flat round dense')
    ...
    async def _handle_main_submit(self) -> None:
        ...
        self.text_area.run_method('focus') # Refocus for quick follow-up messages.
...
class ChatPanelUI:
    ...
    def _build(self) -> None:
        ...
        with ui.card().classes('w-full h-full shadow-lg rounded-lg p-0 flex flex-col overflow-hidden'):
            ...
            with ui.row().classes('items-center w-full p-2 bg-gray-100 dark:bg-gray-800 border-b dark:border-gray-700 flex-shrink-0'):
                ...
                ui.label("Ankdesh AI Framework").classes('text-lg font-semibold flex-grow text-center mr-8')
            ...
            chat_box_action_buttons = [
                {'icon': 'add_photo_alternate', 'on_click': self._handle_add_file_action, 'tooltip': 'Add file'}
            ]
            with ui.element('div').classes('w-full flex-shrink-0 border-t dark:border-gray-700'):
                ChatBox(on_submit=self._handle_user_submission, action_buttons=chat_box_action_buttons)
    ...
    def add_message(self, header: str, text: str) -> None:
        ...
        This method resets the streaming state and applies specific formatting based on the header.
        """
        self._last_message_header = header
        self._current_streaming_content = ""  # Reset content for this message

        # Apply different formatting based on the header.
        if header == "code:python":
            formatted_text = f"```python\n{text}\n```"
        else:  # Default to "text" or any other header type.
            formatted_text = text
        ...
        with self.chat_messages_container:
            with ui.card().classes('w-full no-shadow border dark:border-gray-700'):
                ...
                with ui.card_section().classes('py-2 px-2'):  # Consistent padding for all
                    self._current_streaming_element = ui.markdown(formatted_text).classes('text-sm dark:prose-invert max-w-none')

        self._scroll_to_bottom()

    def stream_message_chunk(self, header: str, token: str) -> None:
        """
        Streams a chunk (token) of a message.

        Appends to the last message if the header is the same. For "code:python", 
        manages formatting with code fences. For other headers, streams plain text.
        """
        """
        ...
        elif header == "code:python":
            self._current_streaming_content += token
            # Maintain code fencing while streaming
            self._current_streaming_element.content = f"```python\n{self._current_streaming_content}\n```"
        else:
            # Streaming text (no code formatting)
            self._current_streaming_content += token
    ...
    async def _handle_user_submission(self, message: str) -> None:
        ...
        # 2. Trigger the SSE client to fetch data from the backend.
        #    This will run in the background.
        self.ui_manager.trigger_sse_orchestration()
        print(f"User message displayed: {message}")
