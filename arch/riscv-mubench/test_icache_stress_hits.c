...
# include "rand_ten_thousand_indices_from_1to1000_array.h"

void main() {
    printf("\n Stress Hits Test --- \n");

    // Fill all 1000 cachelines
    for (int i = 0; i < 1000; ++i) {
        capacity_test_funcs[i]();
    }

    // Keep making hits randomly to these cachelines
    for (int i = 0; i < 10000; ++i) {
        capacity_test_funcs[rand_ten_thousand_indices_from_1to1000[i]]();
    }
}
